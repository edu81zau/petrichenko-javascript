...
// Импортируем createSelector из Redux Toolkit для мемоизированных селекторов
...
const HeroesList = () => {
    ...
    /**
     * `filteredHeroesSelector` - Мемоизированный селектор, созданный с помощью `createSelector`.
     * Он пересчитывает отфильтрованный список героев только в том случае,
     * если изменились его входные зависимости: `activeFilter` или `heroes`.
     * Это оптимизирует производительность, предотвращая ненужные перерасчёты.
     */
    const filteredHeroesSelector = createSelector(
        // Первый входной селектор: получаем активный фильтр из состояния `filters`
        (state) => state.filters.activeFilter,
        // Второй входной селектор: получаем полный массив героев из состояния `heroes`
    ...
    // Используем селектор для получения отфильтрованного списка героев
